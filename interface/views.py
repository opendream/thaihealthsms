...
def view_administer_users_programs(request, sector_id):
	programs = Project.objects.filter(sector__id=sector_id, prefix_name=Project.PROJECT_IS_PROGRAM)

	return HttpResponse(serializers.serialize('json', programs, fields=('id', 'name')))

@login_required
def view_administer_users_projects(request, sector_id):
	projects = Project.objects.filter(sector__id=sector_id, prefix_name=Project.PROJECT_IS_PROJECT, parent_project=None)

	return HttpResponse(serializers.serialize('json', projects, fields=('id', 'name')))
